    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['result:other:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['result:other:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listOther, getOther, delOther, addOther, updateOther } from "@/api/result/other";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除其他上传编号为"' + ids + '"的数据项？').then(function() {
        return delOther(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('result/other/export', {
        ...this.queryParams
