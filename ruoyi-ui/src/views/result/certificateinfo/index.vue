<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="授予机构" prop="issuingInstitution">
        <el-input
          v-model="queryParams.issuingInstitution"
          placeholder="请输入授予机构"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['result:certificateinfo:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['result:certificateinfo:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['result:certificateinfo:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['result:certificateinfo:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="certificateinfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="唯一标识" align="center" prop="id" />
      <el-table-column label="档案标题" align="center" prop="title" />
      ...
      <el-table-column label="授予机构" align="center" prop="issuingInstitution" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="授予机构" prop="issuingInstitution">
          <el-input v-model="form.issuingInstitution" placeholder="请输入授予机构" />
        </el-form-item>
...
<script>
import { listCertificateinfo, getCertificateinfo, delCertificateinfo, addCertificateinfo, updateCertificateinfo } from "@/api/result/certificateinfo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCertificateinfo(this.queryParams).then(response => {
        this.certificateinfoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getCertificateinfo(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCertificateinfo(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCertificateinfo(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除档案资料上传编号为"' + ids + '"的数据项？').then(function() {
        return delCertificateinfo(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('result/certificateinfo/export', {
        ...this.queryParams
      }, `certificateinfo_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
